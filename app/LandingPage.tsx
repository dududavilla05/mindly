...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(124,58,237,0.2)" }}>
      <button
...
function PricingCard({
  ...
  return (
    <div style={{
      ...
      <div style={{ color: highlighted ? "#a78bfa" : "#94a3b8", fontWeight: 600, fontSize: "0.9rem", textTransform: "uppercase", letterSpacing: "0.08em" }}>{name}</div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginTop: 8 }}>
        <span style={{ fontSize: "2.6rem", fontWeight: 800, color: "#f1f5f9" }}>{price}</span>
        {period && <span style={{ color: "#64748b", fontSize: "0.9rem" }}>{period}</span>}
      </div>
      <div style={{ height: 1, background: "rgba(255,255,255,0.07)", margin: "16px 0" }} />
      <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
        {features.map((f, i) => (
          <li key={i} style={{ display: "flex", alignItems: "center", gap: 10, color: f.included ? "#e2e8f0" : "#475569", fontSize: "0.95rem" }}>
            <span style={{ fontSize: "1rem", flexShrink: 0 }}>{f.included ? "✓" : "✗"}</span>
            {f.text}
          </li>
        ))}
      </ul>
      <Link href="/home" style={{
        ...
        background: highlighted ? "linear-gradient(135deg, #7c3aed, #a855f7)" : "rgba(255,255,255,0.06)",
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: "#0a0818", color: "#e2e8f0", fontFamily: "'Inter', -apple-system, sans-serif", overflowX: "hidden" }}>
      {/* ── NAV ── */}
      <nav style={{
        position: "fixed", top: 0, left: 0, right: 0, zIndex: 100,
        padding: "0 24px",
        background: scrolled ? "rgba(10,8,24,0.92)" : "transparent",
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg, #7c3aed, #a855f7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.1rem" }}>✦</div>
            <span style={{ fontWeight: 800, fontSize: "1.2rem", color: "#f1f5f9" }}>Mindly</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Link href="/login" style={{ padding: "8px 18px", borderRadius: 10, color: "#94a3b8", textDecoration: "none", fontSize: "0.9rem", fontWeight: 500 }}>Entrar</Link>
            <Link href="/home" style={{
      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "120px 24px 80px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 800, margin: "0 auto" }}>
          ...
          <h1 style={{ fontSize: "clamp(2.4rem, 6vw, 4rem)", fontWeight: 900, lineHeight: 1.1, margin: "0 0 24px", color: "#f8fafc" }}>
            ...
            <span style={{ background: "linear-gradient(135deg, #7c3aed, #a855f7, #ec4899)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>com Inteligência Artificial</span>
          </h1>

          <p style={{ fontSize: "clamp(1rem, 2.5vw, 1.25rem)", color: "#94a3b8", lineHeight: 1.7, margin: "0 0 40px", maxWidth: 620, marginLeft: "auto", marginRight: "auto" }}>
          ...
          <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
            <Link href="/home" style={{
              padding: "16px 36px", borderRadius: 14, fontWeight: 700, fontSize: "1.05rem",
              background: "linear-gradient(135deg, #7c3aed, #a855f7)",
              color: "#fff", textDecoration: "none",
            ...
            <Link href="/login" style={{
              padding: "16px 36px", borderRadius: 14, fontWeight: 600, fontSize: "1.05rem",
          ...
          <div style={{ display: "flex", gap: 40, justifyContent: "center", marginTop: 64, flexWrap: "wrap" }}>
            {[["10.000+", "lições geradas"], ["98%", "satisfação"], ["6", "ferramentas de aprendizado"]].map(([n, l]) => (
              <div key={l} style={{ textAlign: "center" }}>
                <div style={{ fontSize: "1.8rem", fontWeight: 800, color: "#a78bfa" }}>{n}</div>
                <div style={{ fontSize: "0.85rem", color: "#64748b", marginTop: 2 }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 64 }}>
              <h2 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 800, color: "#f1f5f9", margin: "0 0 16px" }}>O problema com o aprendizado tradicional</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, marginBottom: 64 }}>
            {[
              ...
              <Reveal key={i} delay={i * 100}>
                <div style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 16, padding: "28px 24px" }}>
                  ...
                  <p style={{ color: "#6b7280", margin: 0, lineHeight: 1.6, fontSize: "0.95rem" }}>{p.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <Reveal>
            <div style={{ background: "linear-gradient(135deg, rgba(124,58,237,0.15), rgba(168,85,247,0.08))", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 20, padding: "40px 36px", textAlign: "center" }}>
              <div style={{ fontSize: "2.4rem", marginBottom: 16 }}>✦</div>
              <h3 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#f1f5f9", margin: "0 0 12px" }}>O Mindly resolve tudo isso</h3>
              <p style={{ color: "#94a3b8", fontSize: "1.05rem", lineHeight: 1.7, maxWidth: 580, margin: "0 auto" }}>
      ...
      <section style={{ padding: "100px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 64 }}>
              <div style={{ display: "inline-block", background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: 20, padding: "4px 16px", marginBottom: 16 }}>
                <span style={{ color: "#a78bfa", fontSize: "0.85rem", fontWeight: 600 }}>6 ferramentas poderosas</span>
              </div>
              <h2 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 800, color: "#f1f5f9", margin: "0 0 16px" }}>Tudo que você precisa para aprender</h2>
      ...
      {/* ── HOW IT WORKS ── */}
      <section style={{ padding: "100px 24px", background: "rgba(124,58,237,0.04)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 72 }}>
              <h2 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 800, color: "#f1f5f9", margin: "0 0 16px" }}>Como funciona</h2>
      ...
      <section style={{ padding: "100px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 64 }}>
              <h2 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 800, color: "#f1f5f9", margin: "0 0 16px" }}>Planos para cada momento</h2>
      ...
      <section style={{ padding: "100px 24px", background: "rgba(255,255,255,0.01)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 64 }}>
              <h2 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 800, color: "#f1f5f9", margin: "0 0 16px" }}>O que dizem nossos alunos</h2>
            ...
            {testimonials.map((t, i) => (
              <Reveal key={i} delay={i * 100}>
                <div style={{
                  ...
                  <p style={{ color: "#94a3b8", lineHeight: 1.7, margin: "0 0 20px", fontSize: "0.97rem", fontStyle: "italic" }}>&ldquo;{t.text}&rdquo;</p>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 56 }}>
              <h2 style={{ fontSize: "clamp(1.8rem, 4vw, 2.6rem)", fontWeight: 800, color: "#f1f5f9", margin: "0 0 16px" }}>Perguntas frequentes</h2>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          <Reveal>
            <div style={{
              ...
              <div style={{ position: "absolute", top: -60, right: -60, width: 240, height: 240, borderRadius: "50%", background: "radial-gradient(circle, rgba(124,58,237,0.2) 0%, transparent 70%)", pointerEvents: "none" }} />
              <div style={{ fontSize: "3rem", marginBottom: 20 }}>✦</div>
              <h2 style={{ fontSize: "clamp(1.8rem, 4vw, 2.5rem)", fontWeight: 900, color: "#f8fafc", margin: "0 0 16px" }}>
                Pronto para aprender<br />de verdade?
              </h2>
              <p style={{ color: "#94a3b8", fontSize: "1.1rem", margin: "0 0 36px", lineHeight: 1.7 }}>
      ...
      <footer style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "48px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 24, marginBottom: 32 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 9, background: "linear-gradient(135deg, #7c3aed, #a855f7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem" }}>✦</div>
              <span style={{ fontWeight: 800, fontSize: "1.1rem", color: "#f1f5f9" }}>Mindly</span>
            </div>
            <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
              {[["Entrar", "/login"], ["Começar", "/home"], ["Planos", "#pricing"]].map(([label, href]) => (
